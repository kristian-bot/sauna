...
const navItems = [
  { href: '/hytte/kalender', label: 'Kalender' },
  { href: '/hytte/book', label: 'Book' },
  { href: '/hytte/mine', label: 'Mine bookinger' },
];

const adminItems = [
  { href: '/hytte/admin', label: 'Godkjenning' },
  { href: '/hytte/admin/medlemmer', label: 'Medlemmer' },
];

export function HytteShell({
  children,
  isAdmin = false,
}: {
    ...
    <div className="flex min-h-screen">
      ...
      >
        <div className="p-6 flex items-center justify-between">
          <div>
            <Link href="/hytte/kalender" className="text-lg font-bold">
              Familiehytter
            </Link>
            <p className="text-xs text-white/60 mt-1">Hatteland</p>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
          ...
          {items.map((item) => (
            <Link
              ...
              className={`block px-3 py-2 rounded-lg mb-1 text-sm transition-colors ${
                pathname === item.href || (item.href !== '/hytte/admin' && pathname.startsWith(item.href))
